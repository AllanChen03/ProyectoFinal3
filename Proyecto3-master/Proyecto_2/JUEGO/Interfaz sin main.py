...
ventana = Tk()
ventana.title("Race Death")

Frame1 = Frame(ventana,width=500, height=500)

Frame1.pack()

imagenFondo= PhotoImage(file="image.png")

cargarFondo= Label(Frame1,image=imagenFondo).place(x=0, y=0)

CrearUsuario=Label(Frame1, text="Crear Usuario")

CrearUsuario.place(x=220,y=200)

Proyecto=Label(Frame1, text="Proyecto Taller")

Proyecto.place(x=0, y=480)

IngresarUsuario=Entry(Frame1)

IngresarUsuario.place(x=195, y=235)

def abrirjuego():
    Main()
    

botonStart=Button(ventana,text="Start", command=abrirjuego)
